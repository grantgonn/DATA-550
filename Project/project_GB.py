...
def objective(trial, X, y):
    ...
    md = GradientBoostingClassifier(**params)
...
with mlflow.start_run(run_name='GB Optuna') as run:
    ...
    # Log metrics
    mlflow.log_metric('optuna_cv_f1', GB_f1)
    mlflow.log_metric('best_threshold', best_threshold)
    mlflow.log_metric('threshold_f1', threshold_f1)
    
    # log tags
    mlflow.set_tags(tags={'Project': 'Project GB Optuna',
                         'Opimizer': 'Optuna',
...
model = GradientBoostingClassifier(**GB_params).fit(X, y)
...
with mlflow.start_run(run_name='GB Optuna FS') as run:
    for i in range(5,21):
        ...
        #define the model
        model_new = GradientBoostingClassifier(**GB_params)
        ...
        if n_f1 > GB_f1:    
            ...
            # log tags
            mlflow.set_tags(tags={'Project': 'Project GB Optuna FS',
                                'Opimizer': 'Optuma',
                                'Model_family': 'Gradient Boosting',
                                'feature_version': 2})
            mlflow.end_run()
