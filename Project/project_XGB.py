...
with mlflow.start_run(run_name='XGB Optuna') as run:
    ...
    # Log metrics
    mlflow.log_metric('optuna_cv_f1', XGB_f1)
    mlflow.log_metric('best_threshold', best_threshold)
    mlflow.log_metric('threshold_f1', threshold_f1)
    
    # log tags
    mlflow.set_tags(tags={'Project': 'Project XGB Optuna',
                         'Opimizer': 'Optuma',
...
with mlflow.start_run(run_name='XGB Optuna FS') as run:
    for i in range(5,21):
        ...
        #define the model
        model_new = XGBClassifier(**XGB_params)
        ...
        if n_f1 > XGB_f1:    
            ...
            # log tags
            mlflow.set_tags(tags={'Project': 'Project XGB Optuna FS',
                                'Opimizer': 'Optuma',
                                'Model_family': 'XGBoost',
                                'feature_version': 2})
            mlflow.end_run()
